	...
	$.ajax({
		...
		success: function (result,status,xhr) {
			if (result != null && result.success==true && result.data>0) {
				if(st==0) {
					$('#userstatus').html("<a onclick='ChangeStatus(\""+uid+"\",1)'><span class='label label-warning'>Suspended</span></a>");
					$('#suc_msg').parents().removeClass('hide');
					$('#suc_msg').text("Blocked Successfully");
				}
				else if(st==1) {
					$('#userstatus').html("<a onclick='ChangeStatus(\""+uid+"\",0)'><span class='label label-success'>Approved</span></a>");
					$('#suc_msg').parents().removeClass('hide');
